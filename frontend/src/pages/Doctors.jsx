    ...
    <div>
     ...
     <button className={`py-1 px-3 border rounded text-sm transition-all sm:hidden ${showFilter ? " bg-primary text-white": ""}`} onClick={()=>setShowFilter(prev=> !prev)}>Filters</button>
      <div  className={`flex flex-col gap-4 text-sm text-gray-600 ${showFilter ? 'flex': " hidden sm:flex"}`}>
        ...
        <p onClick={()=>speciality=='Gestroenterologist'?navigate('/doctors'): navigate('/doctors/Gestroenterologist')} className={`w-[94vw] sm:w-auto pl-3 py-1.5 border border-gray-300 rounded transition-all cursor-pointer ${speciality ==='Gestroenterologist'?"bg-indigo-100 text-black": ""} `}>Gestroenterologist</p>
      </div>
      <div className=' w-full grid grid-cols-auto gap-4 gap-y-6'> 
        ...
        <div className=' p-4'>
        <div className={`' flex items-center gap-2 text-center text-sm ${item.available ?'text-green-500' :'t text-gray-500'} '`}>
